...
class MaximumLengthValidator:
    ...
    @staticmethod
    def get_help_text(pronoun="your"):
        return _(f"{pronoun.capitalize()} password must contain " f"no more than {max_length} characters")
    ...
    def validate(self, password, user=None):
        ...
        for char in self.special_chars:
            ...
            if not has_specials_chars:
                raise ValidationError(
                    self.get_help_text(pronoun="this"), code="password_missing_special_chars"
                )

    def get_help_text(self, pronoun="your"):
        return _(f"{pronoun.capitalize()} password must contain at least one of the following special characters: "
