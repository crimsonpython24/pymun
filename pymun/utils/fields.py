...
class MultilingualField(models.Field):
    ...
    def contribute_to_class(self, cls, name, private_only=False, virtual_only: object = False):
        def translated_value(self) -> object:
            language = get_language()
            val = self.__dict__.get(MultilingualField.localized_field_name(name, language))
            if not val:
            ...
            if self.localized_field_model:
                for lang_code, lang_name in settings.LANGUAGES:
                    ...
                    localized_field.contribute_to_class(cls,
                                                        MultilingualField.localized_field_name(name, lang_code))
