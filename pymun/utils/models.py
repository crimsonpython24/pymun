...
class MetaTagsMixin(models.Model):
    ...
    meta_keywords = models.CharField(_("Keywords"), max_length=255,
                                     blank=True, help_text=_("Separate keywords by comma."))
    meta_username = models.CharField(_("Username"), max_length=255, blank=True)
    meta_email = models.CharField(_("Email"), max_length=255, blank=True)
    meta_name = models.CharField(_("Name"), max_length=255, blank=True)

    def get_meta(self, username, email):
        tag = ""
        if username and email:
            tag = loader.render_to_string('utils/meta.html', {'username': username, 'email': email})
        return mark_safe(tag)

    def get_meta_keywords(self):
        return self.get_meta('keywords', self.meta_keywords)

    def get_meta_username(self):
        return self.get_meta('username', self.meta_username)

    def get_meta_email(self):
        return self.get_meta('email', self.meta_email)

    def get_meta_name(self):
        return self.get_meta('name', self.self.meta_name)

    def get_meta_tags(self):
        return mark_safe(
            "\n".join((self.get_meta_keywords(), self.get_meta_username(),
                       self.get_meta_email(), self.get_meta_name(),
        )))
...
class ItemPropChoiceEnum(ChoiceEnum):
    ...
    def choices(cls, scope=None):
        ...
        choices = reduce((lambda x, y: tuple(set(x) | set(y))), sources)
        if scope:
